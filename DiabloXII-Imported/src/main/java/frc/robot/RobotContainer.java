...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    opJoystick.square().whileTrue(new setShooterMode(shooterSubsystem, shooterMode.outtakingFast));
    //Wrist Controls on DPAD
    opJoystick.povDown().toggleOnTrue(new setWristMode(wristSubsystem, wristMode.home));
    opJoystick.povRight().toggleOnTrue(new setWristMode(wristSubsystem, wristMode.low));
    opJoystick.povUp().toggleOnTrue(new setWristMode(wristSubsystem, wristMode.mid));
    //Intake Controls
    ...
    opJoystick.R1().toggleOnTrue(new delayButton(intakeSubsystem, shooterSubsystem));
  }
